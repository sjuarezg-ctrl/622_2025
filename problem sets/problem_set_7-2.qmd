...
Answer: Given that the p-value is lager than the 0.05, then we can accept the null hypothesis that there is no relationship between age and enjoyment of watching golf.
...
datasummary_crosstab(
  education ~ attention_to_politics,
  data = nes2024 |> drop_na(education, attention_to_politics)
)
...
```{r}
nes_clean <- nes2024 |>
  filter(!is.na(attention_to_politics))

chisq.test(nes_clean$education, nes_clean$attention_to_politics)
