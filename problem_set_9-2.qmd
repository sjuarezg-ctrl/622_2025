...
The regression results indicate that turnout levels are higher in states outside the South and in states that offer election day Registration. States outside the South had a higher turnout (about 5.5 percentage points) on average when compared to Southern states which were lower in their turnout.

::: callout-note
... holding election day registration and their high school graduation rates constant.
:::

States that offered election day registration Election-Day Registration had a higher turnout (about 5.8 percentage points) on average when compared to states who did not.

::: callout-note
See above.
:::

Education also plays a role, for every one-point in a state’s high school graduation rate, turnout rises by about 0.10 points which while relatively small is statistically significant.

::: callout-note
See above.

Also, don't compare the estimated effects of IVs measured in different units. This coefficient is smaller because it relates to a one-percentage point change, whereas the others are binary switches.
:::
...
When turnout (vote) is regressed on pcthsg alone, the estimated effect of pcthsg appears more statistically significant than in the fuller model that also includes nonSouth and edr.

::: callout-note
Don't use dataset column names in your write-up. They mean nothing to your reader.
:::

This happens because, in the simple model, pcthsg is capturing not only its own relationship with turnout but also part of the variation that the other variables explain when they are included.
...
::: callout-note
Make your variable names human-friendly.
:::

Answer:

The regression model shows that voter turnout goes down on average slightly over time when compared to earlier years, even after accounting for state registration rules. The coefficient for year is negative and statistically significant, indicating that turnout goes down by roughly 0.046 percentage points per year on average.

::: callout-note
Don't worry about this for now (because we haven't taught it to you), but you need to treat time very carefully in your linear regression models. We need to assume all observations are independent of each other to correctly estimate a linear regression. This assumption does not hold (turnout last election year absolutely impacts turnout this election year).

... holding registration constant.
:::

In contrast, states with higher values on the registration measure show higher turnout on average when compared to states that have a lower registration measure, with an increase of nearly 0.9 percentage points for each unit increase.

::: callout-note
What is the registration measure? What do higher values mean?

... holding the year constant.
:::

The intercept is not substantively meaningful, as it represents the predicted turnout when year = 0, which falls far outside the time period of the dataset. I chose these variables because I was interested in understanding if there would be a noticeable pattern as time would go on, but also because there might be specific years that yield specific results which could require qualitative work and further research.

::: callout-note
To pull out year-specific relationships, you need to convert your `year` to a factor (categorical variable). Otherwise, you are just estimating the constant, linear relationship between time and voter turnout rates. People often do this by decade, or Cold War vs not Cold War.
:::

Registration was also used because it has been a long standing interest of mine to see if registration rates hold any noticeable patterns when held in conversation with turnout.
